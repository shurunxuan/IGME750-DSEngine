...
bool DSSPhysics::Raycast(Ray ray, RaycastHit &mHit)
{
	...
	for (int i = 0; i < boxColliders.size(); i++) {
		...
		float originToDiretion = sqrt(pow((origin.x - direction.x), 2.0f) + pow((origin.y - direction.y), 2.0f) + pow((origin.z - direction.z), 2.0f));
		float originToCenter = sqrt(pow((origin.x - center.x), 2.0f) + pow((origin.y - center.y), 2.0f) + pow((origin.z - center.z), 2.0f));
		float DirectionToCenter = sqrt(pow((direction.x - center.x), 2.0f) + pow((direction.y - center.y), 2.0) + pow((direction.z - center.z), 2.0f));
		float cos_A = (pow(originToCenter, 2.0f) + pow(originToDiretion, 2.0f) - pow(DirectionToCenter, 2.0f)) / (2.0f * originToCenter*originToDiretion);
