...
Texture2D grabTexture : register(t5);

SamplerState basicSampler : register(s0);
SamplerComparisonState shadowSampler : register(s2);
SamplerState refractSampler : register (s3);
...
PixelOutput main(VertexToPixel input)
{
	...
	float refractionIndex = 0.9f;

	float3 refractionDirection = refract(-v, n, refractionIndex);

	float2 refractionUV = mul(float4(refractionDirection, 0.0f), view).xy * 0.1f;
	refractionUV.x *= -1.0f;

	float4 grabColor = grabTexture.Sample(refractSampler, input.screenUV + refractionUV);
	float edgeFactor = abs(dot(v, n));
	float opacity = min(1.0f, 1.0 - edgeFactor);

	result = result * float4(grabColor.rgb, opacity);

	output.Target0 = saturate(result);
	output.Target1 = saturate(result - float4(1.0f, 1.0f, 1.0f, 0.0f));
	output.Target2 = float4(input.viewPos.xyz, 1);
	output.Target3 = float4(normalize(input.viewNormal).xyz, 1);
